...
| live (round 7) | built 2026-09-01, not yet sent | `merged260901` | 5,496,283 (623,835 registrable + 4,872,448 hostname) | 3,078,203.9386 | **17.3219%** | archive `DomainDataCollectionTask_202609012125_IvayloStaykov.tar.gz`, sha256 in `live/`; `verify.sh` and the tier-2 rebuild pass from a fresh extraction | first round scored at the calculator's own unit: hostnames beneath held registrables ship as a second file per year, mostly from re-reading artifacts already on disk; the research loop runs unattended on the fleet |
