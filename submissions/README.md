...
| live (round 7) | built 2026-09-02, not yet sent | `merged260901` | 7,400,658 (625,374 registrable + 6,775,284 hostname) | 4,183,364.4129 | **23.5409%** | archive `DomainDataCollectionTask_202609020128_IvayloStaykov.tar.gz`, sha256 in `live/`; `verify.sh` (eleven checks) and the tier-2 rebuild pass from a fresh extraction, all twenty-one result files byte-identical | first round scored at the calculator's own unit: hostnames beneath held registrables ship as a second file per year, mostly from re-reading artifacts already on disk (90.2% `www.` forms, disclosed); five sources admitted by the loop under the standing rule; the research loop runs unattended on the fleet |
