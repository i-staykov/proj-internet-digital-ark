...
Submission-time measurement: For each submission, t_i is the absolute task-assignment interval in days: t_i = max(1, receipt_date_i - task_assignment_date_member). task_assignment_date_member is the date on which that participant first received the task, and receipt_date_i is the date on which the complete submission was received. The interval is measured in whole calendar days, the clock does not reset after later submissions or benchmark releases, and the same official date records and time zone must be used for all participants. A smaller t_i represents a faster submission.

Time-weighted scoring formula: For accepted submission i, S_i = k x (p_i / t_i), with k = 10. Here p_i is the organizer-verified Equivalent-English Domain percentage increase for that submission, expressed as the percentage value (for example, 5 for 5%), and t_i is the absolute task-assignment interval in whole calendar days defined above. Therefore, with the same verified percentage increase, an earlier complete submission receives a higher score.
...
- Common Crawl candidate-source scope: Avoid using Common Crawl corpus data to extract hostnames for supplementing the 1996-2001 candidate pool. Its crawl data are too distant from the 2001 boundary, and a separate project workstream already covers Common Crawl. Common Crawl-derived discoveries are not included in the active candidate pool or candidate-pool scoring for this task. Instead, seek overlooked external sources broadly, including institutional archives, registry historical publications, academic datasets, email and newsgroup archives, historical directories, and scanned materials. This restriction does not change the fixed Common Crawl TLD English-share weight table used solely for Equivalent-English Domain calculation.
...
Wayback CDX remains one of the key infrastructure components for year-specific validation and subsequent webpage acquisition. Its priority use in the current discovery stage is to validate externally discovered domains and active candidate-pool hostnames, or to obtain dated indexes for genuinely new sources. Do not spend substantial retrieval capacity querying registrable domains or hostnames already present in the benchmark merely to enumerate their subdomains. Those subdomains will be encountered during the later CDX acquisition stage, so performing the same expansion now is duplicative and has a poor return for unknown-domain discovery. The public IA CDX API is currently in use; 504 errors or rate limits during high-value validation do not justify declaring the approach infeasible. Adjust batch sizes, concurrency, pagination, and retry strategies as needed. Explanatory materials must state which CDX acquisition tools were used, which new sources or candidate sets were validated, how requests were batched, the success rate, how failures were handled, and how many previously unknown registrable domains and evidence-backed hostname-year records were added.
...
**VIII. Efficient Unknown-Domain Discovery and Source-Saturation Learning**

Primary discovery target: prioritize registrable domains that are absent from the current benchmark and validate the active candidate pool. Systematically querying existing benchmark domains through IA CDX solely to enumerate subordinate hostnames is not a priority method. It mainly expands namespaces that are already known, duplicates a function of the later CDX-download stage, and consumes proxy, query, storage, and review capacity without materially improving unknown registrable-domain coverage.

Use IA CDX primarily to validate exact externally discovered hostnames or candidate-pool entries for a target year, and to assess newly found historical sources. Preserve the exact queried hostname, target year, returned timestamp, request URL or template, source-response location, and validation outcome. CDX validation should promote qualified candidates into annual files; it should not become a general subdomain enumeration pass over domains already present in the benchmark.

Acceptance of independently discovered hostnames: an exact subdomain hostname obtained directly from an external dataset, historical source, link graph, directory, or other independent discovery route may still be accepted when it has qualifying year-specific evidence. This does not authorize automatically deriving, generating, or bulk-enumerating subdomains from existing benchmark domains. Report newly discovered registrable-domain coverage separately from hostname-level additions so the practical discovery value remains visible.

Exceptional platform investigation should proceed only when an independently discovered source or documented hypothesis indicates that it can produce material benchmark-external value that will not be obtained automatically in downstream CDX acquisition. Record a small pilot's previously unknown registrable-domain yield, hostname yield, overlap, retrieval cost, and evidence quality before scaling. Path-based identities such as /~user/ remain paths rather than hostnames and are never annual hostname records.

For every exceptional archive query approved after a pilot, preserve the source hypothesis, seed, target year, complete query URL or request template, source-response location, retained timestamp, normalization rule, overlap, registrable-domain yield, hostname yield, cost, and failure state. Low or redundant yield must redirect effort toward new external sources and candidate-pool validation rather than trigger wider enumeration of known domains.
...
Use resumable per-year archive queries for candidate-pool validation and genuinely new external source families. Save query templates, target years, pagination or resume checkpoints, retained capture timestamps, source locations, normalization decisions, and failure states. Do not turn the current benchmark into a root-seed list for broad subdomain enumeration. Incomplete high-value validation queries remain scheduled work rather than negative evidence.
...
**XII. Evidence-First Acquisition and Reproducible Provenance**

For archive-based validation of externally discovered hostnames and new source families, execute separate, resumable queries for the required target years. Preserve the exact hostname, retained capture timestamp, complete request template or query URL, source-response location, and pagination or resume state. Broad enumeration beneath existing benchmark domains is not a discovery priority and must not replace unknown registrable-domain discovery or candidate-pool validation. A query must be paginated to completion before its validation yield is treated as measured; a row cap, incomplete page sequence, or timeout is a work-state, not negative evidence.

A bulk or restricted historical dataset may support annual acceptance only when its documented schema binds the exact hostname to a target-year condition that is sufficient for website presence. The evidence ledger must retain the holding institution, dataset title and edition, access and reuse conditions, original or recovery URL, file identity and hash where available, record location, year-bearing field, field definitions, acceptance condition, extraction and normalization code, and a public per-record verification route or query template where one exists. Assess authority from the custodian's responsibility for the data, documented date semantics, stable file identity, transparent schema, and independent verification options. If these conditions are uncertain, retain the material as candidates rather than annual facts.

Do not infer a hostname variant from another form. For example, a capture of `example.com` does not establish `www.example.com`, and a capture of `www.example.com` does not establish the bare hostname. Each form may enter an annual file only with exact-host, year-specific evidence. Derived or unresolved variants must remain in a separately labelled review collection with their derivation and source provenance, so they can be excluded or verified later without affecting the annual master files.

Treat undated or incompletely dated discoveries as durable research assets, not as dead ends. Record their evidence level, source provenance, normalization status, verification route, and unresolved reason in the candidate system. Reconcile that system after every annual merge: promote only records with the required direct evidence, remove candidates already accepted annually, and retain unresolved records for future automated checking. The shared source-feedback ledger must record coverage, yield, overlap, noise, cost or effort, failures, and the next investigation decision so that both positive and negative findings improve subsequent autonomous discovery.

# XIII. Mandatory Evidence Classification and Hostname Integrity Gate

## Annual-master evidence rule

The annual master is a website-evidence product. A hostname-year record may enter `1996.txt` through `2001.txt` only when the retained evidence establishes the exact hostname's web presence in the target year. Qualifying patterns include an exact-host Internet Archive CDX capture with the captured URL/hostname and target-year timestamp retained, a dated webpage snapshot, a dated web link-graph record that identifies the target hostname, or a trusted custodian's documented per-host/year non-error web-capture extract. The evidence must be sufficient for the exact hostname, not merely for a parent domain or another hostname variant.

DNS observations, registry/RDAP/WHOIS registration events that do not establish web presence, email or Usenet delivery headers, and textual mentions are valuable discovery or hostname-in-use evidence, but do not by themselves establish a website for an annual master. Store them as source-specific candidate assets with provenance and a follow-up verification route. They may be promoted only when paired with direct, exact-host, target-year website-level evidence. The same classification applies to every contributor and every source family.

## Mandatory hostname-integrity gate

Before any annual or active-candidate record is merged, scored, or exported, normalize it to lowercase and apply both syntactic and semantic checks. The record must be a hostname (not a URL, path, email address, filename, or IP literal), must meet the project hostname syntax rule, and must end in a current IANA root-zone TLD or a documented historical TLD allowlist. A regular-expression pass alone is not sufficient.

Quarantine rather than accept a bare `www.\<single-label\>` value, an unrecognized terminal TLD, or a hostname whose terminal suffix is immediately duplicated by parsing or construction error (for example, `host.example.org.example.org`). A bare hostname and its `www.` form are distinct records: neither may be inferred from the other. Accept each only when its own exact-host, year-specific evidence is retained.

Every validation run must emit a machine-readable exclusion ledger with at least: hostname, annual year or candidate scope, source file, source line or record location, exclusion reason, normalization decision, and, where available, the original evidence reference. Rebuild results by filtering from the authoritative evidence store; do not manually patch output files. Candidate-pool reconciliation must remove any hostname already accepted in a cleaned annual master.

## Legacy-data remediation

For inherited records without a complete item-level ledger, immediately quarantine records that fail the mandatory hostname-integrity gate and retain the exclusion ledger with the repaired release. A broader legacy evidence re-audit remains required before such records are used as authoritative website facts. Do not silently treat legacy presence as an exemption from the exact-host and target-year rules.
