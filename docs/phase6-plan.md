...
## The email body, written and ready

`fill_report.py` regenerates `private/email-draft.md` and restores its two hand-written stubs, so the text lives here. Paste it back after any re-fill. It is written for the threshold being met, because that is the only condition under which we send.

### OPENING LINE (paste over the first `<!-- ROUND 6` stub)

This round passes the 5% threshold you set.

### SUBSTANCE (paste over the second `<!-- ROUND 6` stub)
