...
| 2026-09-03 | The collector host's login, as user and address, is in seven files of this repository's PUBLISHED history, reachable from nine branches already on the public remote. Found while checking whether the `log/<branch>` tags were safe to push, so the tags did not cause it and were held | open, awaiting the owner's remediation choice; rotation is cheaper and more reliable than rewriting published history, since clones, forks and caches keep what a rewrite removes | `private/security/`, and the branch and file list in the note there. Neither the user nor the address is repeated in any tracked file, which is the rule this predates |
