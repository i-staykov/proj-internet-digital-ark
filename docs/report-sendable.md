...
| 3. Increment | **6,113,548** records |
| 4. Equivalent-English increment | **3,485,235.0631** |
| 5. Equivalent-English growth rate | **19.6124%** |
...
|------------------------------|-------------------|---------:|-------------:|-------:|
| registrable domains, the prioritized unit | `additions/NNNN.txt` | 623,966 | 328,791.0573 | 1.8502% |
| valid hostnames beneath registrables, accepted 2026-09-01 | `hostnames/NNNN_hostnames.txt` | 5,489,582 | 3,156,444.0058 | 17.7622% |
...
| 1999 | 6,345,942 | 46,194 | 843,041 | 7,235,177 | 508,769.3410 |
| 2000 | 10,705,102 | 27,486 | 1,539,721 | 12,272,309 | 864,528.7391 |
| 2001 | 10,536,760 | 500,321 | 2,327,319 | 13,364,400 | 1,632,068.4631 |
| **Total** | **33,848,926** | **623,966** | **5,489,582** | **39,962,474** | **3,485,235.0631** |

**Cumulative score 51.0351%**: the sum of the increases you awarded (1.659986%, 10.730988%, 14.901054%, 4.130718%) plus this round's 19.6124%, as your update log of 2026-08-18 defines it. Round 1 is held out: awarded on records, before the equivalent-English metric.
...
2. **Domain-wide CDX sweeps over subdomain platforms: 1,043,187 EE, 1,423,101
...
3. **Registrable domains, the prioritized unit: 328,791.0573 EE, 623,966 records.** NYPW TimeMaps
   ...
   (71,388) and 22 small dated artifacts (40,369). 46,148 of
...
| `ia_cdx_domain_sweep` | hostname | the row's own 14-digit capture timestamp | 1,423,101 | 1,043,187 |
...
| 27 further sources | both | one row each in `sources.md` and `audit/source_contribution.csv` | 125,108 | 75,938 |
| **Total** | | | **6,113,548** | **3,485,235** |
...
**Composition.** 89.6% of the hostnames are `www.` forms of a registrable. They are distinct
...
- **Saturation ledger**, as your 0831 update asks: `audit/source_saturation_ledger.csv`, one row
  per source family and version. **474 source families searched and recorded** in `sources.md`: 62 developed, 412 evaluated and closed with the measurement that closed them, so the same ground is not broken twice.
...
| **accepted increment** | **6,113,548** | **3,485,235.0631** |
| post-merge total | 39,962,474 | 21,255,823.9657 |

- Overlap with the baseline: **0 records**, so all 6,113,548 submitted count once; the two unit files are disjoint in every year.
