...
diffed against your `candidate_pool.txt` and all six annual files. [CANDREG] are registrable names
whose only dated evidence Section XIII screens out of the annual files (registry zone and drop
lists, dated directories, author mail hosts), re-tracked here rather than lost; [CANDHOST] are
hostnames in two provenance-linked collections: [CANDISC] ISC survey observations, candidate-only
under your 0905 ruling (`isc_survey_hostnames/`), and [CANDHDR] hosts whose only dated evidence is
...
for the candidate track; exact-host captures beside the [CANDHOSTEE] EE of ISC candidates and the
