...
- **The standing rule reads the fleet's verdict on each bound and tests only the new class itself** (CLAUDE.md, Autonomy). `standing_rule.py` parks a source unless its lead's `standing` block says the fleet admitted it with every clause ok (size, terms, robots, class, window), and a park names the clause and the evidence the fleet wrote for it. Whether the terms permit the read is the fleet's `terms` clause, tested against the bounds in its `policy.json` `standing`; a class with no master source in the register yet parks and reaches Ivo.
...
- A clone with no `data/ark.duckdb` can run only the code half. `ark export` and `ark check` fail
  there on a missing database or table (`database does not exist`, or `Table with name ... does not
  exist`), and that is the store being absent rather than an invariant failing: big data never
  reaches git, so a fresh clone never has one.
