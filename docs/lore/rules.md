...
- **A fleet leg downloads only through `scripts/harness/fetch.py`** (#80, 2026-09-09), never its own `curl`: it reads the whole robots.txt of the host in the download URL, refuses a by-name group wherever it sits, honours `Retry-After`, caps at 1 GB by `Content-Length` and again by the stream, writes only under `$ARK_PROBE_DIR`, extracts nothing to disk, and prints the bytes and sha256 the finding quotes. An artifact over the cap or of a risky type is a row in the fleet's `downloads.md` and waits for a human to merge the decision.
