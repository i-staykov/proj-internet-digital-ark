...
| Net-new (domain, year) pairs | **1,313,172** |
| Baseline domains (read-only) | 4,824,656 |
| Total domains in store | 5,288,024 |
| Total (domain, year) pairs in store | 8,180,085 |
| Evidence rows | 11,070,873 |

**Net-new (domain, year) pairs by year:**

| 1996 | 1997 | 1998 | 1999 | 2000 | 2001 |
|--:|--:|--:|--:|--:|--:|
| 97,590 | 1,039,097 | 13,631 | 27,985 | 55,197 | 79,672 |
...
| RDAP gap-fill (sandwich gaps) | `whois_creation` | 1996–2001 | +0 | +10,437 | fills held domains' missing years (present in Y and Y+2); ~38% hit over 15k queried; thin years |
...
- **Evidence rows by type:** `prior_reused` 6,866,913 · `cdx_timestamp` 2,310,422 · `artifact_listing` 1,682,024 · `whois_creation` 171,085 · `link_source` 39,454 · `dated_directory` 975.
...
**Second run — gap-fill.** The same engine fills *held* domains' missing in-window years: a domain present in year Y and Y+2 but missing Y+1 almost certainly existed in Y+1, and its RDAP registration interval supplies the evidence. Of **470,816** such "sandwich-gap" domains, **15,000 were run in two batches: 5,676 dated (~38% hit)** — a filled gap is a net-new pair (the baseline lacked that year) — for **+10,437 net-new pairs concentrated in the thin years** (1998 +2,245, 1999 +2,286, 2000 +2,780). The hit rate far exceeds the link-target run because these domains provably existed around the gap, so more survive. The remaining ~455k sandwich-gap candidates are a documented lever (RDAP-rate-bound: ~3–4k pairs per 10k queried). `ark check` passes after every run.
