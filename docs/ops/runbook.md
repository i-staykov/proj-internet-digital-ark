...
The parent sweeps are held off by the `human` pause flag, because they enumerate subdomains of
parents he holds, which his 0918 text calls duplicative; the platform lanes run outside the
supervisor and stop on their own flag, `~/ark/state/pause-platform`. launchd owns the supervisor, so
beside `just hold`, which stops the job itself, there are only three words:
