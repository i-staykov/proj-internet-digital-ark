...
| High-weight ccTLD RDAP beyond the six already probed (2026-08-27) | **`.ca` is live, serves an in-window creation date, and stops on a licence page we cannot read.** The IANA bootstrap gives endpoints for `ca`, `au`, `in`, `uk` and `sg`; `.au`, `.in`, `.nz`, `.za`, `.ie` and `.us` were already closed on an earlier probe. `.ca` was not. One query to `https://rdap.ca.fury.ca/rdap/domain/rita.ca` returns `('registration', '2001-02-01T17:11:06Z')`, the same `whois_creation` semantics already approved. Ceiling **~25,377 EE**, from 103,541 held in-window `.ca` domains at `.ca`'s 0.8365 and the 29.3% in-window rate measured on Nominet. **It is blocked on terms, not on evidence**: the record carries a Legal Notice binding use to CIRA's Terms of Use, and that page answers HTTP 403 behind a Cloudflare challenge, so the terms cannot be read and will not be worked around. `robots.txt` is 8 lines, names no agent and disallows only `/wp-admin/`, `/?s=` and `/search/`, so robots is not the obstacle. Filed as `cira_ca_rdap / whois_creation`, pending, needing either a human reading of the Terms or a letter of the RIPE kind. `.sg` is untried and worth 31,793 held pairs, far less. |
