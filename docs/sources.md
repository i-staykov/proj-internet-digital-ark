...
| Convention on Biological Diversity meeting documents (2026-08-27) | NOT closed and NOT priced, and the blocker is unusual enough to record: the host is alive and imposes nothing. `www.cbd.int/robots.txt` returns an "Under Construction" HTML page rather than a robots file, so there are no directives, and `/doc/` and `/decisions/cop/?m=cop-04` both serve real pages. Pricing is blocked on enumeration instead: `/doc/?meeting=COP-04` is a 9,334-byte JavaScript shell with no document links in the HTML, so the in-window list has to come from the meetings API first. Requeued. |
