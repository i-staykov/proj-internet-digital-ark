    ...
    if mine:
        ...
        for r in mine:
            if not r.get("url"):
                unlinked += 1
                continue
            figure = "measured" if r.get("measured") else "scout"
            name = f"[`{r['slug']}`]({r['url']})"
            dates = _cell(r.get("dates") or "not quoted", 160)
            terms = _cell(r.get("terms") or "none recorded", 80)
            out.append(
                f"| {r['low']:,.0f} | {figure} | {r['track']} | {r.get('grain') or '?'} | "
                f"{r['ask']} | {name} | {dates} | {terms} |"
            )
        if unlinked:
            out.append("")
            out.append(
                f"{unlinked} lead(s) above the floor carry no artifact link and are not listed: "
                "a source without a fetchable artifact is a hope, not a row."
            )
    if not checked:
        out += [
            "",
