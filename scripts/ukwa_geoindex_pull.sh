...
#
# Usage: bash scripts/ukwa_geoindex_pull.sh [margin_mb]
...
if [ ! -f data/raw/ukwa/geoindex_members.json ]; then
    ...
    uv run python scripts/ukwa_geoindex_map.py >> "$LOG" 2>&1
fi
...
for member in $members; do
    ...
    uv run python scripts/ukwa_geoindex_stream.py "$member" \
