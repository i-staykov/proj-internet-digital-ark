...
#
#     bash scripts/harness/agent_fanout.sh <deadline_epoch> [parallel] [hypothesis_file]
...
ee_now() {
    uv run python scripts/round/round_figures.py 2>/dev/null \
...
while [ "$(date +%s)" -lt "$DEADLINE" ]; do
    ...
    done < <(uv run python scripts/harness/pick_hypotheses.py "$HYPO" $((PAR * PER_RESEARCHER)) --pending-dir "$FIND" 2>/dev/null)
    SLUGS="${SLUGS# }"
...
EOG
    ...
    for idx in $(seq 0 $((PAR - 1))); do
        ...
        if ! uv run python scripts/harness/researcher_brief.py \
...
1. Run: uv run python scripts/harness/screen_hypothesis.py --list-closed
...
3. For your best 2 or 3, re-price on the CURRENT screen. The question is always:
   ...
   Use scripts/pricing/price_items.py. P(store lacks 2001 | held) is 0.611 com, 0.653 net,
   0.568 org, 0.309 uk, so a 2001-dated artifact is worth far more than a 1999 one.
4. Also run: uv run python scripts/harness/reprobe_closed.py --limit 40
...
3. uv run python scripts/harness/rank_triage.py
