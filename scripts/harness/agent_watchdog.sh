...
#
#     bash scripts/harness/agent_watchdog.sh <deadline_epoch> [parallel]
...
while [ "$(date +%s)" -lt "$DEADLINE" ]; do
    if ! pgrep -f "agent_fanou[t].sh" >/dev/null 2>&1; then
        ...
        nohup bash scripts/harness/agent_fanout.sh "$DEADLINE" "$PAR" private/agent-hypotheses.md \
    ...
    if ! pgrep -f "compound_split[s].sh" >/dev/null 2>&1; then
        ...
        nohup bash scripts/engines/compound_splits.sh "$COLLECTOR_DEADLINE" 1500 \
            > /dev/null 2>&1 < /dev/null &
    fi
    sleep 120
done
