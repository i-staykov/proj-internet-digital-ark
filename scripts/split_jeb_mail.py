...
`scripts/parse_jeb_mail.py` therefore only takes a host with an `@` in front of it, a scheme,
...
Read-only against the store.
    ...
    uv run python scripts/split_jeb_mail.py             # report only
...
def main() -> int:
    ...
    if not IN.exists():
        print(f"no journal at {IN}; run scripts/parse_jeb_mail.py over the extracted files first")
        return 1
