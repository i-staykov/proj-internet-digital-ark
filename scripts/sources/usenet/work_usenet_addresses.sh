...
#
#     bash scripts/sources/usenet/work_usenet_addresses.sh <deadline_epoch> [batch] [workers]
...
bank() {
    ...
    if ! uv run python scripts/sources/usenet/split_usenet_addresses.py \
...
for SRC in data/raw/usenet_bulk data/raw/usenet_new; do
    ...
    while [ "$(date +%s)" -lt "$DEADLINE" ]; do
        ...
        if ! ARK_USENET_SRC="$stage" uv run python scripts/sources/usenet/collect_usenet_addresses.py \
