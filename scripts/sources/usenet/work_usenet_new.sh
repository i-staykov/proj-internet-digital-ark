...
#
#     bash scripts/sources/usenet/work_usenet_new.sh <deadline_epoch> [batch] [workers]
...
#
#     ARK_USENET_SRC=data/raw/usenet_bulk bash scripts/sources/usenet/work_usenet_new.sh <deadline>
...
while :; do
    ...
    if ! uv run python scripts/sources/usenet/split_usenet.py "${pending[@]}" --tag "$tag" \
