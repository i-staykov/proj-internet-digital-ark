...
#
#     bash scripts/engines/compound_splits.sh <deadline_epoch> [pause_seconds]
...
while [ "$(date +%s)" -lt "$DEADLINE" ]; do
    ...
    if uv run python scripts/engines/build_promotion_journals.py --tag "$tag" --write >> "$LOG" 2>&1; then
        bash scripts/harness/bank_promotion.sh "$tag" >> "$LOG" 2>&1 \
    ...
    for triple in "usenet_addr:data/raw/usenet_addr:usenet_addr" \
        ...
        if ! uv run python scripts/sources/usenet/split_usenet_addresses.py --in-dir "$dir" \
