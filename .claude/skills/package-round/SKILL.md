...
```
just ship-approved <round>    # bank, regenerate the report and the .docx, then ship
just ship <round>             # quiesce ingestion, export, gate, package, verify
just verify-delivery          # what a reviewer would check: checksums, pair counts, provenance
```

Rules that bite here:

- Never package by hand. `just package` refuses a dirty tree or a stale `output/`, correctly, and
  a hand-run `ark export` races the ingest loop.
- Report artifacts are regenerated and committed BEFORE packaging, which `just ship-approved`
  does in that order.
...
- The last word on the totals is his own calculator, which `just ship-approved` runs with
  `--verify`.
