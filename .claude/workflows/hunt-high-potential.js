...
BEFORE PROPOSING ANYTHING, run \`just find <term>\` by NAME and by POPULATION: every closed family
is a row on \`docs/registers/sources-closed.md\` with the measurement that closed it, and re-proposing
one wastes the only thing that matters. Already done today and NOT to be repeated: .ie, .my, .za, .ph, .in, .nz,
...
const results = await pipeline(
  ...
  (found, l) => {
    ...
    return agent(
      ...
      6. Run \`just find <term>\` by name and by population. A family on sources-closed.md is worth nothing.
