...
- `docs/registers/sources.md` is several hundred kilobytes and reading it whole is denied in
...
- The entry format, the write-up length rule and what a `Decision:` line may claim are in
  `docs/lore/rules.md`, section Registers. The approval gate itself is enforced by `ark ingest`.
