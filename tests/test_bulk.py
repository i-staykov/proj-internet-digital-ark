...
CREATE SEQUENCE evidence_seq START 1;
...
def test_migration_upgrades_pre_taxonomy_store() -> None:
    conn = duckdb.connect(":memory:")
    for statement in filter(str.strip, _OLD_SCHEMA.split(";")):
        conn.execute(statement)
    conn.execute("INSERT INTO source (name, kind) VALUES ('prior_task', 'timestamped')")
    ...
    conn.execute(
        ...
        "VALUES ('example.com', 1, 1997, 'prior_reused', '1997.txt')"
    )
    eid = conn.execute("SELECT evidence_id FROM evidence").fetchone()[0]
    conn.execute(
        "INSERT INTO domain_year (domain, assigned_year, evidence_id) "
        f"VALUES ('example.com', 1997, {eid})"
    )
    # the old CHECK list must reject a taxonomy type before the migration
    with pytest.raises(duckdb.Error):
        conn.execute(
            "INSERT INTO evidence (domain, source_id, evidence_year, evidence_type, "
            "evidence_value) VALUES ('example.com', 1, 1997, 'artifact_listing', 'x')"
        )

    init_db(conn)

    # old rows survive, and the taxonomy types are accepted now
    assert conn.execute("SELECT count(*) FROM evidence").fetchone()[0] == 1
    assert conn.execute("SELECT count(*) FROM domain_year").fetchone()[0] == 1
    conn.execute(
        "INSERT INTO evidence (domain, source_id, evidence_year, evidence_type, evidence_value) "
        "VALUES ('example.com', 1, 1997, 'artifact_listing', 'isc-9701')"
    )
    conn.execute(
        "INSERT INTO evidence (domain, source_id, evidence_year, evidence_type, evidence_value) "
        "VALUES ('example.com', 1, 1997, 'link_target', 'graph-row')"
    )
