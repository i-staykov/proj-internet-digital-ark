...
`probe_texts_corpus.domains_in` is the shared extractor for every corpus of prose or OCR this
project prices or ingests. Its TLD whitelist carries `uk` and `au`, so a pattern with no right
boundary matches `www.nctu.edu` inside `www.nctu.edu.tw` and `tuvok.au` inside
`tuvok.au.af.mil`. Both results are well-formed domains, so no store invariant can see them,
and the error runs in the flattering direction twice: the real host is lost, so the pair count
falls, and the invented TLD outweighs the real one (`.edu` 0.9717 against `.edu.tw` 0.1338).

Measured whole-corpus, the old pattern invented 534 names in the trade-press OCR and 1,442 in
the RTFM FAQs, 1,934 distinct; 128 reached the annual files, worth 85.2549 EE, and every one
also carries same-year evidence from another source, so the corroboration split contained the
register and not the pricing.
"""
...
def test_the_ocr_file_name_comes_from_metadata_not_from_the_identifier() -> None:
    ...
    Magazine 031 [1997-06].pdf" carries "Internet Magazine 031 [1997-06]_djvu.txt" beside it,
    and guessing the name called 32 of 33 in-window UK issues unreachable where the metadata
    shows 33. Unreachable and unnamed look identical from outside, so the reachable share
    this script prints is a floor on every corpus it has closed.
    """
    calls: list[str] = []
