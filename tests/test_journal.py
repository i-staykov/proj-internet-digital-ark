...
The documented ingest commands glob `*.jsonl.gz` and one is often issued while a
collector is still running. If the glob matched the live journal, the loader would ledger
the hash of a half-written file and every later ingest of the finished file would fail its
hash check, with the tail of the run unreachable.
"""
...
def test_a_live_journal_grows_on_disk_as_records_are_written(tmp_path) -> None:
    ...
    first block once took 12.7 minutes against a 10-minute window, which reads as a stall
    on a healthy batch. So this asserts what the monitor assumes, with no explicit flush by
    the caller.
    """
    path = _journal(tmp_path)
...
def test_stopping_a_run_does_not_wait_for_its_queued_work() -> None:
    ...
    every domain up front, so the default `ThreadPoolExecutor.__exit__`, which waits for
    all queued tasks, turns SIGTERM into a wait for hundreds of pending HTTP requests: a
    run once kept going for minutes after `pkill` and had to be killed with -9.
    """
...
def test_a_damaged_gzip_block_does_not_stop_the_resume_scan(tmp_path) -> None:
    """One `kill -9` mid-write once stopped both RDAP engines dead. A journal truncated between
    flushes raises `EOFError`; one whose last gzip block is damaged raises `zlib.error`,
    which is not an `OSError`, and the resume scan died on the first of eleven before a
    query went out, reporting "the list is exhausted or the API refused". The good records
    BEFORE the damage must survive, or a 23 MB journal is re-queried for nothing.
    """
