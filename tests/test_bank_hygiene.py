...
Three properties, each of which has cost something once: a dirty clone is refused
**before** anything is fetched or written, a diverged clone is fast-forwarded rather than
merged, and the gate issue is opened once per crossing rather than once per run. The
fourth makes the others safe to schedule: two consecutive runs over unchanged state change
nothing. The git tests use real git in a temporary pair of clones, because what is under
test is exactly what git does with a dirty tree and a diverged branch.
"""
...
def test_the_gate_reads_this_rounds_window_and_not_the_total(tmp_path: Path) -> None:
    ...
    release lags our submission by days: the morning a round opens, the total net-new
    against the newest baseline is still the last round's and would open a gate issue for
    a round that has collected nothing.
    """
    calls = []
    lines = hyg.gate(
        {
...
def test_two_consecutive_banks_with_no_new_data_change_nothing(tmp_path: Path, monkeypatch) -> None:
    """Asserted on the tree and on the calls at once. The first run is the one that acts: it
    prunes what is spent and opens the gate issue. The second is given the same state, and
    both the digest of every path under the root and the list of `gh` calls come back
    unchanged.
    """
    _staging(tmp_path)
    verified_staging(tmp_path, monkeypatch)
