...
def test_refreshing_replaces_a_live_figure_rather_than_freezing_it(tmp_path) -> None:
    """`raise_open` is append-once, so an entry carrying a count freezes at whatever it was
    when first written: the triage mirror once said 11 while 44 were waiting, and nothing
    about the one surface Ivo reads looked stale.
    """
    path = _doc(tmp_path)
