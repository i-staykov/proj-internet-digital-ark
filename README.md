...
| Document | |
|---|---|
| [docs/ding/](docs/ding/) | **the reviewer's own documents, transcribed verbatim from the `.docx` he ships.** His task brief, his update log, his file guide. Where anything here disagrees with them, they win. Generated by `scripts/round/extract_ding_docs.py`; never hand-edited |
...
`scripts/` is organised by role, one directory each:

| | |
|---|---|
| [scripts/harness/](scripts/harness/) | the autonomous research loop: fan-out, watchdog, briefs, the closed-register screen, approvals, banking |
| [scripts/engines/](scripts/engines/) | the archive collectors and their queues, status, and the VPS handover scripts |
| [scripts/pricing/](scripts/pricing/) | measuring any candidate corpus against the store before anyone builds a collector |
| [scripts/round/](scripts/round/) | round figures, the report, the D3 merge, packaging and verifying the delivery |
| [scripts/sources/](scripts/sources/) | one directory per source family: its collector and its splitter live together, and [docs/sources.md](docs/sources.md) holds the know-how |
| [scripts/output_unit_pack/](scripts/output_unit_pack/) | the reviewer-reproducible output-unit measurement sent on 2026-08-31 |
...
| approve | `uv run python scripts/harness/request_approval.py <spec> --journal <j>` | writes a request into [docs/approved-sources-list.md](docs/approved-sources-list.md) that a human can decide in two minutes. `ark ingest` **refuses** a master-eligible class until it is decided |
...
| brief | `uv run python scripts/round/extract_ding_docs.py --package feedback-phase-N` | transcribes the reviewer's `.docx` into [docs/ding/](docs/ding/) with pandoc, adding only a provenance header carrying the source file's sha256. Run it when a new task package arrives. The body is never retyped: a paraphrase of the brief is the one document here that must not exist |
