...
| Document | |
|---|---|
| [docs/ding/](docs/ding/) | **the reviewer's own documents, transcribed verbatim from the `.docx` he ships.** His task brief, his update log, his file guide. Where anything here disagrees with them, they win. Generated by `scripts/extract_ding_docs.py`; never hand-edited |
...
just check       # lint + format-check + tests, then the eleven data invariants
```
...
| ukwa | `bash scripts/fetch_ukwa_linkage.sh --probe` | asks, in one 1 KB request, whether the Wayback replay will serve capture `20221031190607` of the JISC UK Web Domain Dataset host link graph: 20,930,377,408 bytes against the 2^31 we hold, `.uk` at weight 0.9813. It returned HTTP 504 reproducibly on 2026-08-20, so the probe exists to notice a change rather than to retry a known failure. Without `--probe` it streams the file, decompressing in the pipe and keeping only 1996-2001 rows so a dropped stream still leaves what it reached. **Stop the collectors first**: it is a heavy client at the host they use. Reasoning in C-30 |
| geoindex | `scripts/ukwa_geoindex_map.py`, then `scripts/ukwa_geoindex_pull.sh`, then `scripts/ukwa_geoindex_price.py` | the British Library geoindex, 11.2 GB at `bl.iro.bl.uk`, CC Public Domain, ranged GETs. `map` reads the ZIP64 central directory over HTTP without downloading anything; `pull` streams each member's 1996-2001 rows; `price` measures net-new against the store. **Measured at 79,253 net-new pairs and 77,749.1 equivalent-English**, C-31. The streamer counts timestamp decreases and cancels its own early abort the moment it sees one, because nine of the twelve members are sharded and aborting early on one of those reads 5% of it while looking normal. Different host from the collectors, so it runs beside them |
| bl index | `uv run python scripts/bl_repository_index.py` | names every file in the British Library repository, cheaply. `robots.txt` allows `/` and publishes `/resourcelist`, which gives 20,871 `file_set` ids; a HEAD of `/downloads/<id>` returns a 302 whose `Location` carries the filename, so no payload is transferred and no S3 object is touched. Resumable, stops dead on 429 or 503 rather than backing off into a hammer. This is how a second geoindex-sized dataset would be found |
| usenet | `bash scripts/fetch_usenet_hierarchies.sh <epoch>` | downloads the unheld English-facing Usenet hierarchies, largest expected yield first. **Needs no approval**: `usenet_announce / dated_directory` and its siblings are already `master`, so this is collection under an existing decision. Touches `archive.org/download/`, a different service from the `web.archive.org` CDX the collectors meter against, so it runs beside them. Measured worth about 104,000 equivalent-English over roughly 52 GB, C-29, which is an upper bound |
...
evidence, the corroboration split gates anything a human typed, and eleven invariants run on every pass.
...
```bash
just ship            # quiesce ingestion, export, run the eleven invariants, package, verify
```
